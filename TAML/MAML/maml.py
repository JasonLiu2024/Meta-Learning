...
class MAML:
    ...
    def _outer_step(self, task_batch, train):  # pylint: disable=unused-argument
        ...
        for task in task_batch:
            ...
            # ******************* YOUR CODE HERE *********************
            # ********************************************************
            parameters, accuracy_b = self._inner_loop(
                images_support, labels_support, train)

            predictions = self._forward(images_query, parameters)
            loss = F.cross_entropy(predictions, labels_query)
            accuracy_q = util.score(predictions, labels_query)

            outer_loss_batch.append(loss)
            accuracy_query_batch.append(accuracy_q)
            accuracies_support_batch.append(accuracy_b)
